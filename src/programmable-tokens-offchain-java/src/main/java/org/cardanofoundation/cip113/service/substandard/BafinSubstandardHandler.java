 ...
 *
 * BASIC IMPLEMENTATION: Uses same registration logic as dummy for testing purposes.
 * TODO: Implement full Bafin substandard logic with proper parameterization when requirements are finalized.
 */
@Service
...
public class BafinSubstandardHandler implements SubstandardHandler {
    ...
    public RegisterTransactionContext buildRegistrationTransaction(
        ...
        try {
            ...
            // Bafin validators have different script hashes, so they will produce different policy IDs
            ...
            if (errorMsg != null && errorMsg.contains("Failed to compute script cost")) {
                log.warn("Script cost evaluation failed for Bafin transaction. This may be due to validator format differences. Error: {}", errorMsg);
                return RegisterTransactionContext.error("Failed to compute script cost. The bafin validators may need parameterization or have a different format. " +
                        "Please ensure the validators are properly compiled and available. Original error: " + 
                        (errorMsg.contains("DeserialiseFailure") ? "CBOR deserialization failed - validator format may be incompatible" : errorMsg));
            }
